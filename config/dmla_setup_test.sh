...
AIN} and alertmanager.${DOMAIN}."
 ...
 echo "------------------------------------------------------------"
    ...
    echo "------------------------------------------------------------"
    echo "###############################  Waiting 10 seconds for the monitoring and logging container groups to settle in before starting proxy
containers..."
    echo "------------------------------------------------------------"
    sleep 10
    ...
    echo "------------------------------------------------------------"
    echo "############################### Tailing the logs of the nginx-letsencrypt container through the creation of the Diffie-Hellman group an
d the initial setup of your SSL certificates..."
    echo "------------------------------------------------------------"
    echo "xxxxxxxxxxxx Start of logs, please be patient. Presumably you can make some noise on a different shell to help create some entropy duri
ng the creation of the DH parameters..."
    ...
    echo "------------------------------------------------------------"
    echo "############################### Finished - you're all set up. You can now go to grafana.${DOMAIN}, kibana.${DOMAIN}, prometheus.${DOMAI
N} and alertmanager.${DOMAIN} to check out your metrics, logs and alerts. Use cleanup.sh to uninstall the suite."
    echo "------------------------------------------------------------"
